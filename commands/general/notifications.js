...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (interaction.member.roles.cache.has(config.rolesid.videonotification)) {
            //? Rimuove il ruolo
            interaction.member.roles.remove(config.rolesid.videonotification);

            //? Risponde all'interazione
            let embed = new Discord.MessageEmbed()
                .setTitle(`RUOLO RIMOSSO`)
                .setDescription(`Il ruolo <@&${config.rolesid.videonotification}> ti è stato **rimosso**`)
                .setColor(`RED`);
        ...
        if (!interaction.member.roles.cache.has(config.rolesid.videonotification)) {
            ...
            let embed = new Discord.MessageEmbed()
                ...
                .setDescription(`Il ruolo <@&${config.rolesid.videonotification}> ti è stato **aggiunto**`)
                .setColor(`GREEN`);
            interaction.editReply({ embeds: [embed], ephemeral: true });
        }
    }
