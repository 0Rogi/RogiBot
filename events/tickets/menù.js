		...
		if (interaction.customId.startsWith(`CategoryTicket`)) {
			...
			switch (interaction.values[0]) {
				case `serverissues`: {
					...
					database.collection(`Tickets`).updateOne({ id: interaction.member.id, channel: interaction.channel.id }, { $set: { category: `Problemi nel server` } })
				} break
				case `staffquestions`: {
					let embed = new Discord.MessageEmbed()
						.setColor(`YELLOW`)
					...
					let row = new Discord.MessageActionRow().addComponents(menu)
					interaction.update({ embeds: [embed], components: [row] })
					database.collection(`Tickets`).updateOne({ id: interaction.member.id, channel: interaction.channel.id }, { $set: { category: `Domande allo staff` } })
				} break
			}
			...
			switch (interaction.values[0]) {
				case `botbugs`: {
					...
					interaction.update({ embeds: [embed], components: [row] })
					database.collection(`Tickets`).updateOne({ id: interaction.member.id, channel: interaction.channel.id }, { $set: { subcategory: `Ho riscontrato un bug all'interno del bot` } })
				} break
				case `serverbug`: {
					let embed = new Discord.MessageEmbed()
						.setTitle(`👀 Problemi nel server\n🎡 Bug nel server`)
					...
					let row = new Discord.MessageActionRow().addComponents(button1).addComponents(button2)
					interaction.update({ embeds: [embed], components: [row] })
					database.collection(`Tickets`).updateOne({ id: interaction.member.id, channel: interaction.channel.id }, { $set: { subcategory: `Ho riscontrato un bug all'interno del server` } })
				} break
				case `userreport`: {
					let embed = new Discord.MessageEmbed()
						.setTitle(`👥 Domande allo staff\n🧑 Segnala un utente`)
					...
					let row = new Discord.MessageActionRow().addComponents(button1).addComponents(button2)
					interaction.update({ embeds: [embed], components: [row] })
					database.collection(`Tickets`).updateOne({ id: interaction.member.id, channel: interaction.channel.id }, { $set: { subcategory: `Voglio segnalare un utente` } })
				} break
				case `wantstaff`: {
					let embed = new Discord.MessageEmbed()
					...
					let row = new Discord.MessageActionRow().addComponents(button1).addComponents(button2)
					interaction.update({ embeds: [embed], components: [row] })
					database.collection(`Tickets`).updateOne({ id: interaction.member.id, channel: interaction.channel.id }, { $set: { subcategory: `Posso far parte dello staff?` } })
				} break
				case `wantcollab`: {
					let embed = new Discord.MessageEmbed()
						.setTitle(`👥 Domande allo staff\n🤝 Facciamo una collaborazione`)
					...
					let row = new Discord.MessageActionRow().addComponents(button1).addComponents(button2)
					interaction.update({ embeds: [embed], components: [row] })
					database.collection(`Tickets`).updateOne({ id: interaction.member.id, channel: interaction.channel.id }, { $set: { subcategory: `Facciamo una collaborazione?` } })
				} break
				case `Other`: {
					database.collection(`Tickets`).updateOne({ id: interaction.member.id, channel: interaction.channel.id }, { $set: { subcategory: `Altro...` } })
