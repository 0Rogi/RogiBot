...
module.exports = {
    ...
    async execute(member) {
        ...
        member.roles.add(config.rolesid.unverified)

        const embed = new Discord.MessageEmbed()
            .setTitle(`SEI ENTRATO IN ROGI DISCORD`)
            .setDescription(`Benvenuto in **Rogi Discord**!\n\nPer poter avere l'accesso a **tutti i canali**, premi il pulsante verde in <#${config.channelsid.verify}> e verificati!`)
            .setColor(`#808080`)
            .setThumbnail(member.guild.iconURL({ dynamic: true }));
        member.user.send({ embeds: [embed] }).catch(() => { });

        client.channels.cache.get(config.channelsid.verify).send({ content: `<@&963853288351084586>`, allowedMentions: { users: ['963853288351084586'] } }).then(msg => {
            setTimeout(() => {
                msg.delete();
            }, 1000);
        });
