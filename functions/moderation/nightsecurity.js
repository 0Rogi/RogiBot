...
module.exports = async function nightSecurity(enable) {
    ...
    if (enable) {
        ...
        client.channels.cache.get(config.idcanali.generaltxt).send(`La sicurezza nottura è abilitata 🔒`)
    ...
    if (!enable) {
        ...
        client.channels.cache.get(config.idcanali.generaltxt).send(phrases[Math.floor(Math.random() * phrases.length)])
        client.channels.cache.get(config.idcanali.generaltxt).send(`La sicurezza nottura è stata disabilitata 🔓`)
